...
export default function SiboSuccess() {
  ...
  const copy = {
    ...
    subtitle: isHr
      ? 'Oporavak je putovanje. Uči od drugih, prepoznaj rane znakove i sigurno vraćaj namirnice kako bi izgradio otporniji mikrobiom.'
    ...
    successStories: isHr ? 'Priče o uspjehu' : 'Success Stories',
    relapsePrevention: isHr ? 'Prevencija relapsa' : 'Relapse Prevention',
    foodReintro: isHr ? 'Ponovno uvođenje hrane' : 'Food Reintroduction',
    foodReintroText: isHr
      ? 'Raznolik mikrobiom je ključan za dugoročno zdravlje. Nemoj dugoročno ostati na restriktivnoj prehrani.'
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-slate-900/40 border border-slate-800 rounded-2xl p-6 backdrop-blur-sm">
            ...
            <p className="text-sm text-slate-300 leading-relaxed mb-4">
              {isHr
                ? '"Nakon 3 kure antibiotika, ono što me držalo u remisiji bio je strogi razmak od 4 sata između obroka i prokinetik. FODMAP namirnice sam vraćao postupno tijekom 6 mjeseci. Danas mogu jesti luk bez velikih smetnji."'
          ...
          <div className="bg-slate-900/40 border border-slate-800 rounded-2xl p-6 backdrop-blur-sm">
            ...
            <p className="text-sm text-slate-300 leading-relaxed mb-4">
              {isHr
                ? '"Stres je bio moj najveći trigger. Kad bi posao postao kaotičan, probava bi stala. Vježbe za vagus (duboko disanje prije obroka) promijenile su mi motilitet."'
            ...
            <ul className="space-y-3">
              <li className="flex items-start gap-3">
                ...
                <span className="text-sm text-slate-300">
                  {isHr ? 'Drži 4-5 sati između obroka kako bi MMC mogao "očistiti" crijevo.' : 'Maintain 4-5 hours between meals to allow the MMC to sweep the gut.'}
                </span>
              </li>
              <li className="flex items-start gap-3">
                <CheckCircle2 className="w-4 h-4 text-indigo-400 mt-0.5 shrink-0" />
                <span className="text-sm text-slate-300">
                  {isHr ? 'Razgovaraj s liječnikom o dugoročnoj upotrebi prokinetika.' : 'Discuss long-term prokinetic use with your doctor.'}
                </span>
              </li>
              <li className="flex items-start gap-3">
                <CheckCircle2 className="w-4 h-4 text-indigo-400 mt-0.5 shrink-0" />
            ...
            <div className="space-y-4">
              <div className="bg-slate-950/50 rounded-xl p-4 border border-slate-800/50">
                ...
                <p className="text-xs text-slate-400">
                  {isHr ? 'Uvodi jednu novu namirnicu svaka 3 dana i počni s malom količinom.' : 'Introduce one new food every 3 days. Start with a tiny portion (e.g., 1 tsp of avocado).'}
                </p>
              </div>
              <div className="bg-slate-950/50 rounded-xl p-4 border border-slate-800/50">
              ...
              <div className="bg-slate-950/50 rounded-xl p-4 border border-slate-800/50">
                ...
                <p className="text-xs text-slate-400">
                  {isHr ? 'Ako podnosiš, polako povećavaj porcije kroz tjedne.' : 'If tolerated, slowly increase the portion size over weeks.'}
                </p>
              </div>
            </div>
          </div>
